                                ...
                                <x-dropdown>
                                    ...
                                    <x-slot name="content">
                                        <x-dropdown-link
                                            ...
                                            x-on:click="$wire.extensionId = {{ $php->id }}; $dispatch('open-modal', 'install-extension')"
                                        >
                                            {{ __("Install Extension") }}
                                        </x-dropdown-link>
                                        <x-dropdown-link
                                            class="cursor-pointer"
                                            x-on:click="$dispatch('open-modal', 'update-php-ini')"
                                            wire:click="loadIni({{ $php->id }})"
                                        >
                                            {{ __("Edit php.ini") }}
                                        </x-dropdown-link>
                                        <x-dropdown-link
                                            class="cursor-pointer"
                                            wire:click="restart({{ $php->id }})"
                                        >
                                            {{ __("Restart FPM") }}
                                        </x-dropdown-link>
                                        <x-dropdown-link
                                            class="cursor-pointer"
                                            x-on:click="$wire.uninstallId = {{ $php->id }}; $dispatch('open-modal', 'uninstall-php')"
                                        >
