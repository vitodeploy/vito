    ...
    <x-modal name="add-channel">
        ...
        >
            ...
            <div x-show="['slack', 'discord'].includes(provider)" class="mt-6">
                ...
                <x-text-input value="{{ old('webhook_url') }}" id="webhook_url" name="webhook_url" type="text" class="mt-1 w-full" />
                @error('webhook_url')
                <x-input-error class="mt-2" :messages="$message" />
                @enderror
            </div>
            ...
            <div x-show="provider === 'telegram'" class="mt-6">
                ...
                <x-text-input value="{{ old('bot_token') }}" id="bot_token" name="bot_token" type="text" class="mt-1 w-full" />
                @error('bot_token')
                <x-input-error class="mt-2" :messages="$message" />
                @enderror
            </div>
            ...
            <div x-show="provider === 'telegram'" class="mt-6">
                ...
                <x-text-input value="{{ old('chat_id') }}" id="chat_id" name="chat_id" type="text" class="mt-1 w-full" />
                @error('chat_id')
                <x-input-error class="mt-2" :messages="$message" />
                @enderror
            </div>
