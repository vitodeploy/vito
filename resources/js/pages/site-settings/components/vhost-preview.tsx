...
export default function VHostPreview({ site, children }: { site: Site; children: ReactNode }) {
  ...
  const query = useQuery({
    queryKey: ['site-settings.vhost-preview', site.server_id, site.id],
    queryFn: async () => {
      const response = await axios.get(
        route('site-settings.vhost-preview', {
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="sm:max-w-5xl">
        <SheetHeader>
          ...
          <SheetDescription className="sr-only">Read-only preview of the generated nginx vhost config.</SheetDescription>
        </SheetHeader>
